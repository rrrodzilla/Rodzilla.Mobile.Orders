                ...
                foreach (var document in input)
                {
                    ...
                    switch (order.OrderStatus)
                    {
                        case "awaiting-response":
                            ...
                            //log.LogInformation($"\n\n\n************************* SENDING QUESTION *********************\n\n\n");
                            //message = new JObject(new JProperty("order", JObject.FromObject(order)), new JProperty("action", "awaiting-response"));
                        ...
                        case "canceled":
                            log.LogInformation($"\n\n\n************************* ORDER ESTIMATE ACCEPTED *********************\n\n\n");
                            TwilioClient.Init(Environment.GetEnvironmentVariable("Twilio_Account_Sid"), Environment.GetEnvironmentVariable("Twilio_Auth_Token"));
                            message = new JObject(new JProperty("action", "notify-order-canceled"));
                        ...
                        case "order-started":
                            log.LogInformation(
                                $"\n\n\n********************** ORDER STARTED AT {order.StartTime} ********************\n{order.Customer.FirstName}, we've started your order at Jibe Espresso Bar.  We'll text you when its ready for pickup!\n******************************************\n\n\n");
                            TwilioClient.Init(Environment.GetEnvironmentVariable("Twilio_Account_Sid"), Environment.GetEnvironmentVariable("Twilio_Auth_Token"));
                        ...
                        case "ready-for-fulfillment":
                            log.LogInformation($"\n\n\n*********** ORDER READY TO BE FULFILLED ***************\nOrder Id: {order.Id} for Customer Id: {order.Customer.Id} is ready to be made!\n******************************************\n\n\n");
                            TwilioClient.Init(Environment.GetEnvironmentVariable("Twilio_Account_Sid"), Environment.GetEnvironmentVariable("Twilio_Auth_Token"));
                        ...
                        case "estimated":
                            ...
                            break;
                        case "requested":
                            log.LogInformation($"\n\n\n*********** NEW ORDER REQUEST *******************\n{order.Customer.FirstName} has placed the following order at {order.OriginalOrder.Sent}:\nRequest: {order.OriginalOrder.Message}\n***************************************************\n\n\n");
                            break;
